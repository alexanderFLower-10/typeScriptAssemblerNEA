import{
    Instruction,ThreeParameterInstruction, TwoParameterInstruction, BranchInst, WhiteSpace, Label, HALT
} from './parameterClassDefinitions.js';

export function getInstFromArr(arr: string[]) : Instruction[]{
    let current : string = '';
    const result : Instruction[] = [];
    let splitInst : string[] = [];
        ...
        switch(splitInst.length){
            case 4:
                for(let j = 1; j < splitInst.length-1; j++){
                    ...
                    else throw new Error("Comma not found");
                }
                result[i] = new ThreeParameterInstruction(splitInst[0], splitInst[1], splitInst[2], splitInst[3]); break;
            ...
            case 3:
                ...
                result[i] = new TwoParameterInstruction(splitInst[0], splitInst[1], splitInst[2]); break;
            case 2:
